    ...
    def _llm_chat(self, user_input: str, context: str = "") -> Optional[str]:
        ...
        async def _run() -> Optional[str]:
            return await self.llm.chat(
                ...
                system=(
                    "你是 OpenGuardian，个人数字安全专家。\n"
                    "受众：普通用户（学生/上班族/家长），非技术背景。\n"
                    "回答规范：\n"
                    "1. 口语化但准确——不用术语堆砌，必要时用()白话解释\n"
                    "2. 引用真实标准（CIS/NIST）而非主观臆断\n"
                    "3. 3-6句话，每句独立成段\n"
                    "4. 结尾给1-2条可操作步骤（写清路径/按钮名）\n"
                    "5. 禁止AI模板语（作为AI/请注意/综上所述）\n"
                    "6. 问检测结果时引用当前扫描数据，不编造"
                ),
                temperature=0.7,
            )
